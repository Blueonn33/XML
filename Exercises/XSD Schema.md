...
Сложен текстов елемент може да съдържа текст и атрибути.

## Сложни текстови елементи

Този тип съдържа само просто съдържание (текст и атрибути), затова добавяме елемент `simpleContent` около съдържанието. Когато използваме просто съдържание, трябва да дефинираме разширение ИЛИ ограничение в елемента `simpleContent`
...
:::note
Използваме елемента `extension/restriction`, за да разширим или ограничим базовия прост тип за елемента.
:::

Пример за XML елемент shoesize, който съдържа само текст

<shoesize country="france">35</shoesize>

Примерът декларира `complexType` shoesize. Съдържанието е дефинирано като цяло число, а елементът shoesize съдържа и атрибут с име country
...
Можем също да дадем име на елемента `complexType` и да позволим на елемента shoesize да има атрибут type, който да се отнася до името на complexType
...
# XSD смесено съдържание

Елемент от смесен сложен тип може да съдържа атрибути, елементи и текст

## Сложни типове със смесено съдържание

XML елемент `letter`, който съдържа както текст, така и други елементи
...
Следната XML Schema декларира елемента `letter`
...
:::note
За да се позволи показването на символни данни между дъщерните елементи на `letter`, атрибутът `mixed` трябва да бъде зададен на true.

Тагът <xs:sequence> означава, че дефинираните елементи (name, orderid и shipdate) трябва да се появят в този ред вътре в елемент `letter`
:::

Можем също да дадем име на елемента `complexType` и да позволим на елемента `letter` да има атрибут type, който да се отнася до името на complexType
